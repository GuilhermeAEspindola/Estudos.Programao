...
class square:
    ...
    def pos(self):
        return[self.dim[0]+self.x, self.dim[1]+self.y, self.dim[2]+self.x, self.dim[3]+self.y, self.dim[4]+self.x, self.dim[5]+self.y, self.dim[6]+self.x, self[7]+self.y]


    def update(self):
        if(self.x>0 and self.x<width-grid_size):
            self.x += self.velx
        if(self.y>0 and self.y<width-grid_size):
            self.y += self.vely
        ...
        if(self.y==0 and self.vely>0):
            self.y += self.vely
        if(self.y==width-grid_size and self.vely<0):
            self.y += self.vely
...
class Game:
    ...
    def __init__(self):
        ...
        f = square(random.randint(grid_size,( width/grid_size))*grid_size - grid_size, random.randint(grid_size, (height/grid_size))*grid_size - grid_size, 'purple')
